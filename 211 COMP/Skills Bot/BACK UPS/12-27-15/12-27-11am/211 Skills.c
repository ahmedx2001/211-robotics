...
task Shooter(){
	...
	while (true)
	{
		...
		Velocity = -SensorValue(ShooterEncoder)*10*60*25/360;
		SensorValue(ShooterEncoder) = 0;
		writeDebugStreamLine("Speed: %d", Velocity);
		writeDebugStreamLine("Target: %d", Target);
		...
		if (Target > 1){
			...
			MotorSpeed = MotorSpeed + (error * KP);
