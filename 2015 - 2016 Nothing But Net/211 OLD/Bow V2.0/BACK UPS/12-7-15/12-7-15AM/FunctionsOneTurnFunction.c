...
void Turn (long Angle) {
	...
	while (abs(error) > 10) {
		// P = calculate the error
		error = Angle - abs(SensorValue(TurnGyro));

		// D = Calculate the speed
		Velocity = abs(SensorValue(TurnGyro)) - PreviousAngle;
		...
		BaseSpeed(-(MotorSpeed * Direction), (MotorSpeed * Direction));

		writeDebugStreamLine("Cycle: %d", cycle);

		// Save the distance for the next cycle
		PreviousAngle = abs(SensorValue(TurnGyro));
